...
static const int64 MAX_MONEY = 10000000 * COIN; //The max ammount of coins in existence
...
static const uint256 hashGenesisBlockTestNet("0x000000078daf2b6cb4faddea4adccabf3846436f07bc6497883c215e64ae469a");

static const uint256 hashMerkleRootGBlockOfficial("0x0");
static const uint256 hashMerkleRootGBlockTestNet("0x8bf9f18d31dfbe7c0735d8862260e5c2ee2d0d4e9c736298b1ec49c1f2e63228");
...
int64 GetProofOfStakeReward(int64 nCoinAge, const CBlockIndex* pindex = pindexBest);
...
public:
    ...
    {
        // DCS: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // DCS: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // DCS: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // DCS: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // DCS: whether transaction is a coinstake
    bool fCoinStake;

    // DCS: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // DCS: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // DCS
            undo.nTime = nTime;            // DCS
        }
        return true;
    }
...
public:
    ...
    // DCS: block signature - signed by coin base txout[0]'s owner
    ...
    // DCS: two types of block: proof-of-work or proof-of-stake
    ...
    // DCS: get max transaction timestamp
    ...
    // Context-independent validity checks
    bool CheckBlock(CValidationState &state, bool fCheckPOW=true, bool fCheckMerkleRoot=true, bool fGenesis=false) const;

    // Store block on disk
    // if dbp is provided, the file is known to already reside on disk
    bool AcceptBlock(CValidationState &state, CDiskBlockPos *dbp = NULL);

    bool GetCoinAge(uint64& nCoinAge) const; // DCS: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // DCS: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (DCS proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // DCS: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // DCS: proof-of-stake related block index fields
    unsigned int nFlags;  // DCS: block index flags
    enum
    {
