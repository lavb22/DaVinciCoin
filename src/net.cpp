...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("davincicoin-ext-ip");
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Davincicoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"seed.davincicoin.net", "seed.davincicoin.net"},
    {NULL, NULL}
};
        ...
        if (HaveNameProxy()) {
            ...
            while (pCounter<6){
				...
				if (ngethostbyname(strDNSSeed[seed_idx][1], vaddr))
				{
					...
					printf("Failed to connect to %s, trying again in 20seg (%d/5)\n",strDNSSeed[seed_idx][1], pCounter);
					pCounter++;
				}
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Davincicoin is probably already running."), addrBind.ToString().c_str());
        else
...
{
    printf("StopNode()\n");
    GenerateDavincicoins(false, NULL);
    MapPort(false);
