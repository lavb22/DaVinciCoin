...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "getblockwinner"         && n > 0) ConvertTo<boost::int64_t>(params[0]);
    if (strMethod == "importaddress"          && n > 2) ConvertTo<bool>(params[2]);
#ifdef TESTING
    if (strMethod == "timetravel"             && n > 0) ConvertTo<boost::int64_t>(params[0]);
    if (strMethod == "generatestake"		  && n > 0) ConvertTo<bool>(params[0]);
#endif
    return params;
}
...
std::string HelpMessageCli(bool mainProgram)
{
    ...
    if(mainProgram)
    {
        ...
        strUsage += "  -conf=<file>           " + _("Specify configuration file (default: davincicoin.conf)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Connect to JSON-RPC on <port> (default: 10001 or testnet: 10003)") + "\n";
