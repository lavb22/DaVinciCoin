...
#ifndef DAVINCICOIN_WALLET_H
#define DAVINCICOIN_WALLET_H
...
public:
    ...
    void AvailableCoins(std::vector<COutput>& vCoins, unsigned int nSpendTime, bool fOnlyConfirmed = true, const CCoinControl *coinControl=NULL, const bool fWatch=false) const;
    ...
    bool AddWatchOnly(const CScript& dest, const CKeyID& destID);
    ...
    int64 GetCredit(const CTransaction& tx, const bool fWatch=false) const
    {
        int64 nCredit = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {	if (!CWallet::HaveWatchOnly(txout.scriptPubKey) || fWatch)
...
public:
    ...
    int64 GetCredit(bool fUseCache=true, const bool fWatch=false) const
    {
        ...
        nCreditCached = pwallet->GetCredit(*this,fWatch);
        fCreditCached = true;
        return nCreditCached;
    }
    ...
    int64 GetAvailableCredit(bool fUseCache=true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                const CScript &scriptOut = txout.scriptPubKey;
                if (!pwallet->HaveWatchOnly(scriptOut)){
